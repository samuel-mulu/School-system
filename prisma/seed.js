...
function getScoreByPerformance(performance, maxScore) {
    const ranges = {
...
async function main() {
    ...
    students.forEach((student, index) => {
        const rand = index % 20;
        if (rand < 6)
            studentPerformance[student.id] = 'excellent';
        else if (rand < 12)
            studentPerformance[student.id] = 'good';
        else if (rand < 17)
            studentPerformance[student.id] = 'average';
        else
            studentPerformance[student.id] = 'poor';
    });
