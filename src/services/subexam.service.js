...
export const createSubExam = async (data) => {
    ...
    if (existing) {
        throw new ConflictError(`Sub-exam "${data.name}" already exists for this subject and term`);
    }
...
export const getSubExamsBySubjectAndTerm = async (subjectId, termId) => {
    // Verify subject exists
    const subject = await prisma.subject.findUnique({
        where: { id: subjectId },
    });
...
export const updateSubExam = async (id, data) => {
...
export const deleteSubExam = async (id) => {
    const subExam = await prisma.subExam.findUnique({
        where: { id },
    });
    if (!subExam) {
...
export const validateWeights = async (subjectId, termId) => {
    const subExams = await prisma.subExam.findMany({
        where: {
            subjectId,
            termId,
    ...
    if (!isValid) {
        throw new BadRequestError(`Invalid weight distribution. Sub-exams should total 60% (currently ${subExamTotal.toFixed(2)}%), ` +
...
//# sourceMappingURL=subexam.service.js.map