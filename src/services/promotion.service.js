 ...
 */
export const calculateStudentYearlyAverage = async (studentId, classId) => {
    ...
    const subjectAverages = await Promise.all(subjects.map(async (subject) => {
        try {
...
export const getPromotionPreview = async () => {
    ...
    const students = await Promise.all(activeStudentClasses.map(async (sc) => {
        const overallAverage = await calculateStudentYearlyAverage(sc.studentId, sc.classId);
...
export const promoteStudents = async () => {
    ...
    if (!preview.canPromote) {
        throw new BadRequestError('Cannot promote students. Term 2 must be closed first.');
    }
    ...
    const classMap = new Map(); // gradeId -> classId
        ...
        if (studentPreview.nextGradeId && classMap.has(studentPreview.nextGradeId)) {
            ...
            await prisma.studentClass.create({
                data: {
                    ...
                    reason: studentPreview.outcome === 'PASS'
