...
export const createTerm = async (data) => {
    ...
    if (existing) {
        throw new ConflictError(`Term "${data.name}" already exists for this academic year`);
    }
    // Validate dates
...
export const getTermByName = async (name, academicYearId) => {
    if (academicYearId) {
        // Use findFirst since composite unique constraint might not exist until migration is run
...
export const activateTerm = async (id) => {
    ...
    if (term.academicYear.status !== 'ACTIVE') {
        throw new BadRequestError('Cannot activate term for inactive academic year');
    }
